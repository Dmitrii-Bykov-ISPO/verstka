                ...
                Row(
                  ...
                  children: [
                    ...
                    if (waitingOtp) ...[
                      ...
                      TextButton(
                        onPressed:
                        loading ? null : _reset2FA, // сброс процесса 2FA
...
class HomeScreen extends StatefulWidget { // главный экран после входа, со стейтом (нужно хранить посты и пользователя)
  ...
  const HomeScreen(
      {super.key,
        ...
        required this.themeController}); // конструктор с обязательными параметрами
...
class _HomeScreenState extends State<HomeScreen> { // состояние главного экрана
  ...
  bool isAdmin = false; // флаг "я админ (bykovda)" — нужен для показа кнопки "Пользователи"
  ...
  Future<void> _loadAll() async { // приватный метод загружает пользователя и посты
    ...
    try {
      ...
      isAdmin = loginLower == 'bykovda'; // считаем админом только пользователя с логином bykovda
  ...
  void _stub(BuildContext ctx, String msg) =>
      ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(content: Text(msg))); // показываем временную заглушку через снекбар

  void _openEvents() { // открыть экран событий
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => const EventsScreen())); // пушим экран событий
  }

  void _openSelfAccount() { // открыть "Личный кабинет" (редактирование своего аккаунта)
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => UserEditScreen(
          userId: widget.currentUserId, // редактируем самого себя
          isSelf: true, // флаг "личный кабинет"
        ),
      ),
    );
  }

  void _openProfileStub() { // заглушка для "Профиль"
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Моя страница (заглушка)')), // короткое сообщение
    );
  }

  void _openUsersScreen() { // открыть экран со списком пользователей (только для админа bykovda)
    if (!isAdmin) { // защита от случайного вызова, если не админ
      _stub(context, 'Экран пользователей доступен только администратору'); // показываем сообщение
      return;
    }
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => UsersScreen(
          currentUserId: widget.currentUserId, // передаём id текущего пользователя, чтобы понимать "isSelf"
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) { // строим визуальную часть главного экрана
    final list = loading // если идёт загрузка
      ...
      children: [
        Row(children: [
          const Expanded(
              child: InfoCard(
                  ...
                  title: '10 новостей')), // карточка "новости"
          Expanded(
            child: Stack( // чтобы нарисовать красную точку поверх
              children: [
                ...
                InfoCard(
                  icon: Icons.event,
                  iconColor: myEventsCount > 0
                      ? Colors.green
                      : Colors.grey, // зелёный если >0, иначе серый
                  title:
                  '$myEventsCount событ${myEventsCount == 1 ? "ие" : (myEventsCount >= 2 && myEventsCount <= 4 ? "ия" : "ий")}', // русское склонение
                ),
                if (hasTodayEvent) // если есть событие сегодня
                  Positioned( // рисуем красную точку справа-сверху
                    ...
                    child: Container(
                      width: 10,
                      height: 10, // размер точки
                      decoration: const BoxDecoration(
                          color: Colors.red,
                          shape: BoxShape.circle), // красный кружок
                    ),
                  ),
        ...
        ...posts.expand((p) sync* { // разворачиваем посты в последовательность виджетов: карточка поста + отступ
          yield PostCard.buildFromData( // создаём карточку поста из модели
            ...
            profileAsset: p.authorAvatarUrl ??
                'assets/profile0.jpg', // если нет url аватара — используем локальный ассет-заглушку
        ...
        if (isAdmin) ...[
          const SizedBox(height: 16), // отступ от постов
          ElevatedButton.icon(
            onPressed: _openUsersScreen, // открываем экран списка пользователей
            icon: const Icon(Icons.people), // иконка "люди"
    ...
    return Scaffold( // каркас экрана с app-body и нижней панелью
        ...
        child: Column( // вертикальная колонка
          children: [
            AppHeader( // шапка приложения с приветствием и кнопками
              ...
              onOpenAccount: _openSelfAccount, // "Личный кабинет"
              onOpenProfile: _openProfileStub, // "Профиль" (заглушка)
            ),
      ...
      bottomNavigationBar: BottomAppBar( // нижняя панель с кнопками
        ...
        child: Padding( // внутренние отступы
          ...
          child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [ // распределяем элементы по краям
                Row(children: [ // группа иконок слева
                  IconButton(
                      icon: const Icon(Icons.home),
                      onPressed: () => _goHome(context)), // кнопка "домой"
                  IconButton(
                      icon: const Icon(Icons.search),
                      onPressed: () =>
                          _stub(context, 'Поиск (заглушка)')), // кнопка "поиск" (пока заглушка)
                  IconButton(
                      icon: const Icon(Icons.person),
                      onPressed: () =>
                          _stub(context, 'Профиль (заглушка)')), // кнопка "профиль" (пока заглушка)
                  IconButton(
                      icon: const Icon(Icons.event),
                      onPressed: _openEvents), // кнопка "События"
                ]),
              ]),
        ),
      ),
...
class UsersScreen extends StatefulWidget { // экран со списком всех пользователей (для администратора bykovda)
  final String currentUserId; // final — id залогиненного пользователя (нужно, чтобы понимать "я это или нет")
  const UsersScreen({super.key, required this.currentUserId}); // конструктор с обязательным параметром
...
class _UsersScreenState extends State<UsersScreen> {
  List<UserModel> users = const []; // список пользователей, по умолчанию пустой
  bool loading = false; // флаг индикатора загрузки
  String? error; // текст ошибки (если что-то пошло не так)

  @override
  void initState() { // инициализация
    super.initState(); // базовый initState
    _loadUsers(); // сразу загружаем список пользователей
  }

  Future<void> _loadUsers() async { // загрузка списка пользователей из таблицы users
    setState(() {
      loading = true; // включаем индикатор
    ...
    try {
      ...
      users = (rows as List)
          .map<UserModel>((u) => UserModel( // собираем UserModel
        id: u['id'] as String, // id пользователя
        login: (u['login'] ?? '') as String, // логин или пустая строка
        fullName: (u['full_name'] ?? '') as String, // ФИО или пустая строка
        avatarUrl: u['avatar_url'] as String?, // путь к аватарке (относительный или null)
      ))
          .toList(); // превращаем в список
    } catch (e) {
      error = 'Не удалось загрузить пользователей: $e'; // сохраняем текст ошибки
    } finally {
      if (mounted) setState(() => loading = false); // выключаем индикатор, если виджет ещё смонтирован
    }
  }

  ImageProvider _avatarFor(UserModel u) { // определяем, какую картинку использовать для аватара
    final path = (u.avatarUrl ?? '').trim(); // путь из БД
    if (path.isNotEmpty) { // если путь задан
      return NetworkImage(
        publicUrl(
          bucket: avatarsBucketName, // берём из бакета avatars
          objectKey: path, // относительный путь
        ),
      );
    }
    return const AssetImage('assets/profile0.jpg'); // иначе — локальный ассет-заглушка
  }

  void _openUser(UserModel u) { // открыть экран редактирования конкретного пользователя
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => UserEditScreen(
          userId: u.id, // редактируем этого пользователя
          isSelf: u.id == widget.currentUserId, // если id совпадает — это "я", включаем режим "личный кабинет"
        ),
      ),
    );
  }
  ...
  Future<void> _openCreateUser() async { // открыть экран создания пользователя
    final created = await Navigator.of(context).push<bool>(
      MaterialPageRoute(builder: (_) => const CreateUserScreen()), // пушим экран создания
    );
    if (created == true) { // если когда-нибудь будет возвращаться true — обновляем список
      await _loadUsers(); // перезагрузка списка
    }
  }

  @override
  Widget build(BuildContext context) {
    final Widget body = loading // если идёт загрузка
      ...
      child: Text(
        ...
        : ListView( // иначе показываем список пользователей
      padding: const EdgeInsets.all(16), // отступы
      children: [
        ...List.generate(users.length, (i) { // генерируем элементы списка через List.generate
          final u = users[i]; // текущий пользователь
          final avatar = _avatarFor(u); // картинка для CircleAvatar
          return GestureDetector( // делаем всю строку кликабельной
            onTap: () => _openUser(u), // по тапу открываем редактирование
            child: Container(
              margin: const EdgeInsets.symmetric(vertical: 4), // вертикальные отступы между элементами
              padding: const EdgeInsets.symmetric(vertical: 8), // внутренний отступ по вертикали
              child: Row(
                children: [
                  CircleAvatar(
                    radius: 20, // размер аватарки
                    backgroundImage: avatar, // картинка
                  ),
                      ...
                      children: [
                        ...
                        if (u.fullName.isNotEmpty) // если ФИО не пустое
                          Text(
                            ...
                            style: Theme.of(context)
                                .textTheme
                                .bodySmall, // мелкий шрифт по теме
                          ),
                      ],
                    ),
                  ),
                  const Icon(Icons.chevron_right), // стрелочка "вперёд"
                ],
              ),
            ),
          );
        ...
        // элемент списка "[добавить пользователя]"
        GestureDetector(
          onTap: _openCreateUser, // по тапу — экран создания пользователя
          child: Container(
            padding: const EdgeInsets.symmetric(vertical: 12), // внутренний отступ
            child: Row(
              children: const [
                Icon(Icons.add), // иконка "+"
                SizedBox(width: 8), // отступ
                Text('[добавить пользователя]'), // текст
              ],
            ),
          ),
        ),
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Пользователи')), // заголовок экрана
      body: body, // тело экрана
    );
  }
}
enum _HeaderMenuAction { account, profile }
...
class AppHeader extends StatelessWidget { // виджет шапки (без состояния), показывает приветствие и аватар
  ...
  final VoidCallback onOpenAccount; // открыть "Личный кабинет"
  final VoidCallback onOpenProfile; // "Профиль" (заглушка)

  const AppHeader({ // конструктор шапки
    super.key, // передаём key в базовый класс
    required this.greeting, // обязательные параметры
    required this.currentUserLogin, // обязательный параметр
    required this.currentUserAvatarUrl, // обязательный параметр
    required this.onToggleTheme, // обязательный параметр
    required this.onSignOut, // обязательный параметр
    required this.onOpenAccount, // обязательный параметр
    required this.onOpenProfile, // обязательный параметр
  });
  ...
  ImageProvider _resolveAvatar() { // внутренний метод определяет, откуда грузить картинку аватара
    ...
    if (currentUserAvatarUrl != null && currentUserAvatarUrl!.isNotEmpty) { // если относительный путь внутри бакета
      return NetworkImage(
        publicUrl(
          ...
          objectKey: currentUserAvatarUrl!,
        ),
      ); // собираем публичный url из бакета avatars
    }
    ...
    ) // пробуем дефолтный файл <логин>.jpg
        ...
        children: [
          Expanded( // блок текста занимает доступное место слева
            child: Text( // приветствие
              ...
              style: const TextStyle(
                  fontSize: 24,
                  fontWeight: FontWeight.bold), // крупный жирный шрифт
            ),
          ),
          ...
          Row(children: [ // кнопки справа
            IconButton(
                ...
                icon: const Icon(Icons.brightness_6)), // переключение темы
            IconButton(
                tooltip: 'Выход',
                onPressed: onSignOut,
                icon: const Icon(Icons.logout)), // выход из аккаунта
            PopupMenuButton<_HeaderMenuAction>(
              tooltip: 'Аккаунт',
              onSelected: (action) {
                switch (action) {
                  case _HeaderMenuAction.account:
                    onOpenAccount();
                    break;
              ...
              itemBuilder: (ctx) => const [
                PopupMenuItem(
                  value: _HeaderMenuAction.account,
...
class InfoCard extends StatelessWidget { // простая карточка с иконкой и заголовком
  ...
  const InfoCard(
      {super.key,
        ...
        required this.title}); // конструктор с обязательными параметрами
  ...
  Widget build(BuildContext context) => Container( // корневой контейнер карточки
    ...
    decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(
            16)), // фон по теме и скругления
    child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [ // вертикальная колонка
          Icon(icon, color: iconColor), // иконка нужного цвета
          const SizedBox(height: 8), // отступ
          Text(title,
              style: const TextStyle(
                  fontSize: 18,
                  fontWeight: FontWeight.bold)), // заголовок карточки
        ]),
  );
}
...
class PostCard extends StatelessWidget { // карточка отдельного поста
  ...
  static Widget buildFromData({
    // удобная фабрика для создания PostCard
    ...
    required List<String> photos, // список картинок
      ...
      PostCard(
          ...
          time: time,
          caption: caption,
          photos: photos); // возвращаем инстанс PostCard

  bool _isUrl(String p) => p.isHttpUrl; //  является ли строка http/https url

  void _openPhoto(BuildContext context, String p) => // открывает экран просмотра фото
  Navigator.of(context).push(
      MaterialPageRoute(builder: (_) => PhotoViewScreen(imageAsset: p))); // пушим новый маршрут с PhotoViewScreen
  ...
  Widget _img(BuildContext context, String p, {BorderRadius? r}) { // строим виджет картинки с возможностью тапнуть
    final w = _isUrl(p)
        ...
        : Image.asset(p, fit: BoxFit.cover); // если url — грузим из сети, иначе — из ассетов
    return GestureDetector(
      // ловим тап по картинке
      onTap: () => _openPhoto(context, p), // при тапе открываем полноэкранный просмотр
      child: ClipRRect(
          borderRadius: r ?? BorderRadius.circular(16),
          child: w), // скругляем углы и вставляем изображение
    );
  }
  ...
  Widget _photos(BuildContext context) { // раскладка фото внутри карточки
    ...
    if (photos.length == 1) {
      return SizedBox(
          ...
          child: _img(context, photos[0])); // крупное фото на всю ширину
    }
    if (photos.length == 2) {
      // два фото
      return SizedBox(
        ...
        child: Row(children: [
          // горизонтальное расположение двух фото
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, i) => _img(context, photos[i],
          r: BorderRadius.circular(10)), // каждая ячейка — картинка со скруглением
    );
  }

  @override
  Widget build(BuildContext context) { // собираем карточку поста
    final avatar = _isUrl(profileAsset)
        ? NetworkImage(profileAsset)
        : AssetImage(profileAsset) as ImageProvider; // источник картинки для аватара автора
    return Container(
      // контейнер карточки
      padding: const EdgeInsets.all(16), // внутренние отступы
      decoration: BoxDecoration(
          color: Theme.of(context).cardColor,
          borderRadius:
          BorderRadius.circular(16)), // фоновый цвет из темы и скругления
      child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
              ...
              Column(
                  ...
                  children: [
                    ...
                    Text(name,
                        style: const TextStyle(
                            fontWeight:
                            FontWeight.bold)), // имя автора жирным
                    Text(time,
                        style: TextStyle(
                            color: Theme.of(context)
                                ...
                                .bodySmall
                                ?.color
                                ?.withOpacity(0.6))), // приглушённый цвет для времени
                  ]), // конец колонки
            ...
            Text(caption,
                style: const TextStyle(fontSize: 16)), // текст поста
            const SizedBox(height: 12), // отступ
...
class PhotoViewScreen extends StatelessWidget { // экран просмотра изображения
  ...
  bool get _hasUrl =>
      (imageAsset ?? '').isHttpUrl ||
          (imageAsset != null && imageAsset!.isNotEmpty); // есть ли что показывать

  @override
  Widget build(BuildContext context) { // строим интерфейс просмотра
    final child = Builder(builder: (_) {
      if (imageBytes != null) {
        // если переданы байты — показываем их
        return Center(
      ...
      if (!_hasUrl) {
        ...
        return const Center(
            child: Text('Нет изображения',
                style: TextStyle(color: Colors.white))); // показываем сообщение
      }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        // только кнопка "Назад" через AppBar; без "Домой"
...
class _EventsScreenState extends State<EventsScreen> {
  ...
  Future<void> _load() async { // загрузка событий
    setState(() {
      loading = true;
      error = null;
    }); // включаем индикатор, сбрасываем ошибку
    try {
      ...
      final from = asCalendar
          ...
          : DateTime.now()
          .subtract(const Duration(days: 1)); // для списка берём всё будущее
      final to = asCalendar
  ...
  Future<void> _join(String id) async { // записаться
    try {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Вы записались')));
    } catch (e) {
      // ignore: use_build_context_synchronously
      ScaffoldMessenger.of(context)
  ...
  Future<void> _leave(String id) async { // отказаться
    try {
      ...
      // ignore: use_build_context_synchronously
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Вы отказались от участия')));
    } catch (e) {
      // ignore: use_build_context_synchronously
      ScaffoldMessenger.of(context)
          .showSnackBar(SnackBar(content: Text('Ошибка отказа: $e')));
    }
  }
  ...
  Widget build(BuildContext context) {
    final body = loading // если идёт загрузка
        ...
        ? Center(
        child: Text(error!,
            style: const TextStyle(color: Colors.red))) // показываем ошибку
  ...
  Widget _calendar() { // вид календаря
    ...
    for (final e in events) {
      // пробегаем все события
      final d =
      DateTime(e.startAt.year, e.startAt.month, e.startAt.day); // только дата
      byDay.putIfAbsent(d, () => []).add(e); // добавляем в список дня
    }
    final selectedDay = DateTime(focusedDay.year, focusedDay.month,
        focusedDay.day); // нормализованная выбранная дата
    final ofDay =
        byDay[selectedDay] ?? const <EventModel>[]; // события конкретного дня
    ...
    return Column(
      children: [
        TableCalendar<EventModel>(
          ...
          onPageChanged: (fd) async {
            // при перелистывании месяцев
            focusedDay = fd; // обновляем фокус
            await _load(); // подгружаем события для нового месяца
          },
          selectedDayPredicate: (d) =>
          DateTime(d.year, d.month, d.day) == selectedDay, // выделяем выбранный день
          onDaySelected: (sel, foc) =>
              setState(() => focusedDay = foc), // кликом меняем выбранную дату
        ),
        const SizedBox(height: 8), // отступ
        Expanded(
          // ниже список событий текущего дня
          child: ListView(
            children: ofDay
                .map((e) => EventTile(
              // карточки событий выбранного дня
              e: e,
              onJoin: () => _join(e.id),
              onLeave: () => _leave(e.id),
  ...
  void _openDetails(EventModel e) { // открыть экран деталей
    Navigator.of(context)
        .push(MaterialPageRoute(builder: (_) => EventDetailsScreen(event: e)));
  }
  ...
  Future<void> _openCreateEvent() async { // открыть форму создания
    ...
    if (created == true) {
      // если вернулись с флагом "создано"
...
class EventTile extends StatelessWidget { // карточка события в списке
  ...
  Widget build(BuildContext context) {
    ...
    final coverWidget = cover != null
        ? GestureDetector(
      // по тапу открываем на весь экран
      onTap: () => Navigator.of(context).push(MaterialPageRoute(
          builder: (_) => PhotoViewScreen(imageAsset: cover))),
      child: ClipRRect(
          ...
          children: [
            ...
            Expanded(
              ...
              child: Column(
                  ...
                  children: [
                    Text(e.title,
                        style: const TextStyle(
                            fontWeight: FontWeight.bold)), // название
            ...
            Column(
              ...
              children: [
                if (canJoin)
                  TextButton(
                      onPressed: onJoin, child: const Text('Записаться')),
                if (canLeave)
                  TextButton(
                      onPressed: onLeave, child: const Text('Отказаться')),
              ],
            ),
          ],
        ),
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 12), // отступ
          Text(
            '${EventTile._fmt(event.startAt)} — ${EventTile._fmt(event.endAt)}',
...
class _CreateEventScreenState extends State<CreateEventScreen> {
  ...
  final imageCtrl =
  TextEditingController(); // относительный путь в bucket (заполняется автоматически после загрузки)

  DateTime startAt =
  DateTime.now().add(const Duration(hours: 2)); // дефолт
  ...
  bool get _pickedIsImage { //  является ли выбранный файл картинкой (по расширению)
  ...
  Future<void> _uploadPickedFile() async {
    ...
    try {
      ...
      final objectKey =
          '$uid/${DateTime.now().millisecondsSinceEpoch}.$ext'; // относительный путь внутри бакета
      await supabase
          .storage // загружаем в storage (upsert разрешаем — чтобы можно было перезалить при одинаковом ключе)
          .from(eventImagesBucketName)
          .uploadBinary(objectKey, _pickedBytes!,
          fileOptions: const FileOptions(
              upsert:
              true)); // contentType можно не указывать — Supabase сам попытается определить

      setState(() {
        // кладём путь в поле формы и рисуем предпросмотр
        imageCtrl.text =
            objectKey; // теперь этот путь сохранится в events.image_path
      });
      // подсказка пользователю
      // ignore: use_build_context_synchronously
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Картинка загружена')));
    } catch (e) {
  ...
  Widget _asListPreview() { // виджет мини-превью как в списках событий
    final fake = EventModel(
      // собираем временную модель для превью
      id: 'preview',
      title:
      (titleCtrl.text.isEmpty ? 'Название события' : titleCtrl.text),
    ...
    return IgnorePointer(
      // чтобы превью не кликалось (показываем вид)
      ignoring: true,
      child: EventTile(
        e: fake,
        onJoin: () {},
  ...
  void _openPickedFull() { // полноэкранное открытие выбранной (ещё не загруженной) картинки
    if (_pickedBytes != null && _pickedIsImage) {
      Navigator.of(context).push(
          MaterialPageRoute(builder: (_) => PhotoViewScreen(imageBytes: _pickedBytes)));
    }
  }
    ...
    body: ListView(
      ...
      children: [
        TextField(
            ...
            decoration: const InputDecoration(
                labelText: 'Название*')), // заголовок
        const SizedBox(height: 8),
        TextField(
            controller: descCtrl,
            decoration:
            const InputDecoration(labelText: 'Описание')), // описание
        const SizedBox(height: 8),
        TextField(
            controller: placeCtrl,
            decoration: const InputDecoration(
                labelText: 'Место проведения')), // место
        const SizedBox(height: 8),
        Row(children: [
          Expanded(
            child: TextField(
              ...
              readOnly:
              true, // путь формируется автоматически после загрузки
              decoration: const InputDecoration(
        ...
        if (imageCtrl.text.trim().isNotEmpty) ...[
          ...
          const SizedBox(height: 6),
          GestureDetector(
            // клик по превью — открываем фуллскрин
            onTap: () => Navigator.of(context).push(MaterialPageRoute(
        ...
        Row(children: [
          Expanded(
              child: Text(
                  'Начало: ${EventTile._fmt(startAt)}')), // показываем выбранное начало
          TextButton(
            onPressed: () async {
        ...
        Row(children: [
          Expanded(
              child: Text(
                  'Окончание: ${EventTile._fmt(endAt)}')), // показываем выбранное окончание
          TextButton(
            onPressed: () async {
              ...
              if (picked != null) {
                setState(() => endAt = DateTime(picked.year, picked.month,
                    picked.day, endAt.hour, endAt.minute));
              }
            },
            child: const Text('Дата'),
          ),
            ...
            onPressed: () async {
              ...
              if (picked != null) {
                setState(() => endAt = DateTime(
                    endAt.year, endAt.month, endAt.day, picked.hour, picked.minute));
              }
            },
            child: const Text('Время'),
          ),
...
class UserEditScreen extends StatefulWidget {
  ...
  const UserEditScreen({super.key, required this.userId, required this.isSelf});
...
class _UserEditScreenState extends State<UserEditScreen> {
  ...
  final passwordCtrl =
  TextEditingController(); // новый пароль (только для себя)

  String twoFactorType = 'auto'; // текущий режим 2FA
  String? avatarPath; // относительный путь аватарки в bucket avatars

  Uint8List? _avatarBytes; // байты выбранного файла для аватара
  ...
  static const List<String> _twoFaOptions = [
    'auto',
    'email',
    'none'
  ]; // допустимые значения 2FA

  @override
  void initState() {
    super.initState();
    _load(); // сразу грузим данные
  }

  Future<void> _load() async { // загрузить данные пользователя из таблицы users
    setState(() {
      loading = true;
      error = null;
    });
    try {
      final row = await supabase
          .from('users')
          .select('login, full_name, email, avatar_url, two_factor_type')
          .eq('id', widget.userId)
          .maybeSingle();
      ...
      loginCtrl.text = (row['login'] ?? '') as String;
      fullNameCtrl.text = (row['full_name'] ?? '') as String;
      emailCtrl.text = (row['email'] ?? '') as String;
      avatarPath = row['avatar_url'] as String?;
      twoFactorType = (row['two_factor_type'] ?? 'auto') as String;
    } catch (e) {
  ...
  bool get _avatarIsImage {
    final name = (_avatarName ?? '').toLowerCase();
    return name.endsWith('.jpg') ||
        name.endsWith('.jpeg') ||
        name.endsWith('.png') ||
        name.endsWith('.gif') ||
  ...
  Future<void> _pickAvatar() async { // выбрать файл для аватара
    final res = await FilePicker.platform.pickFiles(
      allowMultiple: false,
      type: FileType.image, // аватарка — картинка
      withData: kIsWeb,
    );
    ...
    if (bytes == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Не удалось прочитать файл аватарки')));
      }
      return;
    }
    setState(() {
  ...
  Future<void> _uploadAvatar() async { // загрузить аватарку в bucket avatars
    ...
    try {
      String ext = 'jpg';
      final name = (_avatarName ?? '').trim();
      final dot = name.lastIndexOf('.');
      if (dot > 0 && dot < name.length - 1) {
        ext = name.substring(dot + 1).toLowerCase();
      }
      // кладём аватарку по пути <userId>/<timestamp>.<ext>
      final objectKey =
          '${widget.userId}/${DateTime.now().millisecondsSinceEpoch}.$ext';
      await supabase.storage
          .from(avatarsBucketName)
          .uploadBinary(objectKey, _avatarBytes!,
          fileOptions:
          const FileOptions(upsert: true));

      setState(() {
        avatarPath = objectKey;
      });

      if (mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('Аватарка загружена')));
      }
    } catch (e) {
      setState(() => error = 'Ошибка загрузки аватарки: $e');
    } finally {
      if (mounted) setState(() => saving = false);
    }
  }

  void _openAvatarFull() {
    if (_avatarBytes != null && _avatarIsImage) {
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => PhotoViewScreen(imageBytes: _avatarBytes),
        ),
      );
  ...
  Future<void> _save() async { // сохранить изменения
    ...
    try {
      ...
      await supabase
        ...
        'avatar_url':
        (avatarPath ?? '').trim().isEmpty ? null : avatarPath,
        'two_factor_type': twoFactorType,
      })
          .eq('id', widget.userId);

      //если это "личный кабинет" — можем обновить auth-профиль (email/пароль)
      if (widget.isSelf) {
      ...
      setState(() => error = 'Не удалось сохранить: $e');
    } finally {
      if (mounted) setState(() => saving = false);
    }
  }
  ...
  Widget build(BuildContext context) {
    final title =
    widget.isSelf ? 'Личный кабинет' : 'Редактирование пользователя';
      ...
      body: ListView(
        ...
        children: [
          ...
          DropdownButtonFormField<String>(
            value:
            _twoFaOptions.contains(twoFactorType) ? twoFactorType : 'auto',
            items: _twoFaOptions
          ...
          Row(
            children: [
              Expanded(
                child: TextField(
                  readOnly: true,
                  controller: TextEditingController(text: avatarPath ?? ''),
                  decoration: const InputDecoration(
                    labelText: 'Путь к аватарке (relative в avatars)',
                  ),
                ),
              ),
            ],
          ...
          Row(
            children: [
              OutlinedButton.icon(
                ...
                label: Text(
                    _avatarName == null ? 'Выбрать аватар' : _avatarName!),
              ),
              const SizedBox(width: 8),
              ElevatedButton.icon(
                onPressed: saving ? null : _uploadAvatar,
                icon: const Icon(Icons.cloud_upload),
                label: const Text('Загрузить в avatars'),
              ),
            ],
          ),
          const SizedBox(height: 8),
          if (_avatarBytes != null ||
            ...
            Text(
              ...
              style: Theme.of(context).textTheme.bodySmall, // мелкий текст-подсказка
            ),
            const SizedBox(height: 6), // небольшой отступ
            GestureDetector(
              onTap: _openAvatarFull, // по тапу открываем полноэкранный просмотр (PhotoViewScreen)
              child: Builder(
                builder: (context) {
                  ImageProvider? img; // сюда положим источник картинки

                  if (_avatarBytes != null && _avatarIsImage) {
                    // если выбрали новый файл и это картинка
                    img = MemoryImage(_avatarBytes!); // используем байты из памяти
                  } else if (avatarPath != null && avatarPath!.isNotEmpty) {
                    // иначе, если есть путь в бакете
                    img = NetworkImage(
                      publicUrl(
                        bucket: avatarsBucketName, // берем из бакета avatars
                        objectKey: avatarPath!, // относительный путь
                      ),
                    );
                  }

                  return CircleAvatar(
                    radius: 40, // круглый аватар 80x80
                    backgroundColor:
                    Theme.of(context).cardColor, // фон по теме
                    backgroundImage: img, // сама картинка (если есть)
                    child: img == null
                        ? const Icon(Icons.person,
                        size: 40) // если картинки нет — иконка пользователя
                        : null, // если картинка есть — иконку не показываем
                  );
                },
              ),
            ),
          ...
          if (widget.isSelf) ...[
            TextField(
              ...
              decoration: const InputDecoration(
                ...
                helperText: 'Если оставить пустым — пароль не изменится',
              ),
            ),
            const SizedBox(height: 12),
          ],
...
//TODO не забыть про  проблему с anonkey
// экран создания пользователя (форма + аватарка, без реального создания auth-пользователя) ===
// ВАЖНО: здесь НЕТ вызова supabase.auth.admin.* — с anon key это невозможно нормально сделать.
...
class _CreateUserScreenState extends State<CreateUserScreen> {
  ...
  String? _avatarName;
  String? avatarPath; // относительный путь в avatars (если загрузили)
  ...
  Future<void> _uploadAvatar() async {
    ...
    try {
      ...
      // пока пользователя ещё нет — кладём во временный префикс "new-users/"
      final objectKey =
          'new-users/${DateTime.now().millisecondsSinceEpoch}.$ext';
      await supabase.storage
          .from(avatarsBucketName)
          .uploadBinary(objectKey, _avatarBytes!,
          fileOptions:
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      // TODO: сделать вызов backend/RPC, который:
      // создаст пользователя в auth (email + пароль),
      // создаст профиль в public.users (login, full_name, two_factor_type, avatar_url),
      // вернёт id нового пользователя.
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text(
              'Создание пользователя пока не реализовано (нужен backend/RPC).'),
        ));
        Navigator.of(context).pop(
            false); // false = реального создания не было
      }
    } catch (e) {
        ...
        children: [
          ...
          TextField(
            ...
            decoration: const InputDecoration(
              labelText: 'Пароль',
              helperText:
              'Пока только поле формы — сохранение пароля на бэке не реализовано',
            ),
          ),
          const SizedBox(height: 12),
          DropdownButtonFormField<String>(
            ...
            items: const [
              DropdownMenuItem(
                  value: 'auto', child: Text('auto (по умолчанию)')),
              DropdownMenuItem(
                  value: 'email', child: Text('email (код по почте)')),
              DropdownMenuItem(
                  value: 'none', child: Text('none (без 2FA)')),
            ],
            onChanged: (v) {
              if (v == null) return;
              setState(() => twoFactorType = v);
            },
            decoration: const InputDecoration(labelText: 'Метод 2FA'),
          ),
          const SizedBox(height: 12),
          Row(
            children: [
              Expanded(
                child: TextField(
                  ...
                  controller: TextEditingController(text: avatarPath ?? ''),
                  decoration: const InputDecoration(
                    labelText: 'Путь аватарки (avatars)',
                  ),
                ),
              ),
            ],
          ...
          if (_avatarBytes != null ||
            ...
            Text(
              'Предпросмотр аватарки:',
              style: Theme.of(context).textTheme.bodySmall, // подсказка
            ),
            const SizedBox(height: 6),
            GestureDetector(
              onTap: _openAvatarFull, // по тапу — полный экран
              child: Builder(
                builder: (context) {
                  ...
                  if (_avatarBytes != null && _avatarIsImage) {
                    ...
                    img = MemoryImage(_avatarBytes!); // берём из памяти
                  } else if (avatarPath != null && avatarPath!.isNotEmpty) {
                    // иначе, если уже загружена в бакет
                    img = NetworkImage(
                      publicUrl(
                  ...
                  return CircleAvatar(
                    radius: 40, // круглый аватар
                    backgroundColor: Theme.of(context).cardColor, // фон
                    backgroundImage: img, // картинка или null
                    child: img == null
                        ? const Icon(Icons.person,
                        size: 40) // заглушка, если нет картинки
                        : null,
                  );
                },
              ),
          ...
          ElevatedButton(
            ...
            child: saving
                ? const CircularProgressIndicator()
                : const Text('Сохранить (UI-заглушка)'),
          ),
        ],
      ),
    );
